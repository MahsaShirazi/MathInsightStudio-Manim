...
class GraphExample(Scene):
    def construct(self):
        ...
        x_labels = axes.get_x_axis().add_labels({0: "0", 5: "5", 10: "10"})
        y_labels = axes.get_y_axis().add_labels({-1: "-1", 4: "4", 8: "8"})
        ...
        # Labels (manually placed instead of axes.plot_label)
        # Use axes.i2gp(x, graph) to put the label near a specific x-value
        sin_label = MathTex(r"\sin(x)", color=BLUE)
        sin_label.next_to(axes.i2gp(9, sin_graph), UP)

        relu_label = Text("ReLU", color=YELLOW)
        relu_label.next_to(axes.i2gp(9, relu_graph), UP)

        step_label = Text("Step", color=RED_A)
        step_label.next_to(axes.i2gp(4, step_graph), UP)
        ...
        self.play(
            ...
            Transform(sin_label, relu_label),
        )
        self.wait()
        ...
        self.play(
            ...
            Transform(sin_label, step_label),
        )
        self.wait()

        # Parabola
        parabola = axes.plot(lambda x: 0.25 * x**2, color=BLUE)
        ...
        # Dot on parabola at x = 2
        dot = Dot(color=RED)
        dot.move_to(axes.i2gp(2, parabola))
        self.play(FadeIn(dot, scale=0.5))

        # ValueTracker to move the dot along the parabola
        x_tracker = ValueTracker(2)

        dot.add_updater(
            lambda d: d.move_to(axes.i2gp(x_tracker.get_value(), parabola))
        )
...
class SurfaceExample(ThreeDScene):
    def construct(self):
        ...
        # Add meshes
        for mob in surfaces:
            mob.shift(IN)  # push forward in z
            # SurfaceMesh may not be available in all manim builds; use a safe fallback
            try:
                mesh = SurfaceMesh(mob)
            except Exception:
                # Fall back to an empty group so later code that expects .mesh won't fail
                mesh = VGroup()
            mesh.set_stroke(BLUE, 1, opacity=0.5)
            mob.mesh = mesh  # attach for later

        surface = surfaces[0]

        # ----------- First surface appear -----------
        self.play(
            FadeIn(surface),
        ...
        # Rotate all others for alignment
        ...
        # ----------- Morph torus1 → torus2 with camera move -----------
        self.play(
            Transform(surface, surfaces[2]),
            self.camera.animate.set_theta(self.camera.theta - 20 * DEGREES),
            self.camera.animate.set_phi(self.camera.phi - 10 * DEGREES),
            run_time=3
        )
        ...
        self.add_fixed_in_frame_mobjects(light_text)
        ...
        light_dot = Dot3D(radius=0.2, color=WHITE)
        light_dot.add_updater(lambda d: d.move_to(light.get_center()))
        self.add(light_dot)

        # Move the light
        self.play(light.animate.move_to(3 * IN), run_time=3)
        self.play(light.animate.shift(6 * OUT), run_time=3)

        # ----------- Drag text -----------
        drag_text = Text("Try pressing d or f and dragging the mouse")
        self.add_fixed_in_frame_mobjects(drag_text)
